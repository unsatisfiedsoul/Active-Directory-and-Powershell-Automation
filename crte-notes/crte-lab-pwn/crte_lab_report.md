...
## 01 - STUDENT.US.TECHCORP.LOCAL

### **Enumeration**

***

STUDENT131 is a domain computer on which we have authenticated credentials. We can bypass AMSI and then load _Powerview.ps1_ to start the bare enumeration on the system.
...
**1. To Enumerate name of group in RestricedGroups in&#x20;**_**Mgmt**_**&#x20;Group Policy**
...
* There is a misconfigured service named _'AMG'_, we can abuse it by adding our selves as an local admin.
...
## 02 - US-MGMT.US.TECHCORP.LOCAL

### **OU Delegation**

***

* We will be doing an OU delegation from _studentuser131_ to get access to _US-MGMT_
* Opening the BloodHound Report we can see that _studentuser131_ has few properties which can be abused to access _US-MGMT_ machine
...
1. We can add our selves into the _MACHINEADMINS_ Group
...
3. Now that we have access to _us-mgmt_, we can execute mimikatz and dump the hashes
...
## 03 - US-MAILMGMT.US.TECHCORP.LOCAL

### **LAPS Abuse**

***

1. We will load _PowerView.ps1_ and start enumerating for users whose creds we can read in clear text
...
2. From the above response we can see that _us-mailmgmt_'s creds can be read in clear text. Using the AD Module we can view the laps permissions for that user and then dump the creds.
...
3. Using the creds we can use `winrs` to logon as _US-MAILMGMT_
...
## 04 - US-JUMP.US.TECHCORP.LOCAL

### **Local Admin Access**

***

1. From the hash extracted from _US-MAILMGMT_ we found `jumpone` user. Using PS session as that user and search for local admin's present on that system.
...
* The certificate is owned by _pawadmin_.
...
## 06 - US-DC.US.TECHCORP.LOCAL

### **UnConstrained Delegation - Print Bug**

***

1. Doing enumeration we can see that _US-WEB_ has unconstrained delegation on _US-DC_
...
2. We can use print bug exploit to capture the ticket of the DC. For this we can spawn two sessions as _US-WEB_ where `rubeus.exe` will be set as monitor mode and on the other session we can use `MS-RPRN.exe` to force the _DC_ to connect to _WEB_ and capture the ticket.
...
2. Using _Rubeus_ we can inject the ticket into the session
...
3. Now we can perform a DC-Sync attack and dump the _krgtgt_ and _administrator_ hash
...
4. With the administrator hash we can use pth and get a session to _US-DC_
...
> Paste the Private key in a file named : _cert2.pem_
...
| 1104 | MSOL\_16fb75d0227d | c1db8cdcb7a89f56dd00b77e384c2c9c |
...
## 07 - US-HELPDESK.US.TECHCORP.LOCAL

### **RBCD Abuse**

***

* After dumping hashes from _US-MGMT_ we obtained the ntlm hash for _mgmtadmin_
* On the BloodHound Report we can use RBCD abuse to access _US-HELPDESK_
...
## 08 - US-ADCONNECT.US.TECHCORP.LOCAL

### **Local Admin Access**

***

1. _helpdeskadmin_ has local admin access to _US-ADCONNECT_. We can spawn a session _helpdeskadmin_ and use _New-PSSession_ to connect with it.
...
* _adconnect_ user has local admin access to _US-ADConnect_

1. Load _Powerview.ps1_ into the session and enumerate the PHS account and server where AD Connect is installed
...
**38. Which attach/technique can be executed against techcorp.local due to high privileges of MSOL\_16fb75d0227d**
...
* With the help of azure creds we can perform an dc-sync attack on _techcorp.local_
...
## 10 - US-MSSQL.US.TECHCORP.LOCAL

### **Constrained Delegation Abuse**

***

* From _US-Jump_ we extracted _appsvc_ creds. _appsvc_ has constrained delegation to _US-MSSQL_. We can verify this using the BloodHound Report !\[\[Pasted image 20230702000034.png]]
...
3. Now we can logon to _US-MSSQL_
...
4. We can now access the shares on _BASTION-DC_
...
5. Setting a listener and creating a schedule task we can spawn a shell as _BASTION-DC_
...
## 12 - PRODUCTION-DC.PRODUCTION.LOCAL
...
## 13 - USVENDOR-DC.USVENDOR.LOCAL

### **UnConstrained Delegation - Print Bug**

***

1. We can use print bug exploit to capture the ticket of the DC. For this we can spawn two sessions as _US-WEB_ where `rubeus.exe` will be set as monitor mode and on the other session we can use `MS-RPRN.exe` to force the _USVENDOR-DC_ to connect to _WEB_ and capture the ticket.
...
1. Create a golden ticket as an Enterprise Admin on _US.TECHCORP.LOCAL_ .
...
9. Spawn a session with the admin creds and get a session as _EU-DC_
...
4. Now we can access the _eushare_
...
## 16 - EUVENDOR-NET.EUVENDOR.LOCAL
...
4. Access _EUVendor-Net_ using _winrs_
...
## 17 - DB-SQLPROD.DB.LOCAL -> 18 - DB-SQLSRV.DB.LOCAL
...
6. So, we do have database links to other SQL Servers. If xp\_cmdshell is enabled (or rpcout is true that allows us to enable xp\_cmdshell), it is possible to execute commands on any node in the database links using the below commands
...
9. Because the link from DB-SQLProd to DB-SQLSrv is configured to use sa. We can enable RPC Out and xp\_cmdshell on DB-SQLSrv! Run the below commands on the reverse shell we got above. Ignore the scary looking message after the first command:
...
* Download and execute mimikatz in memory on _DB-SQLSRV_
...
* Download and execute mimikatz in memory on _DB-SQLProd_
